...
class KDTree
{
  ...
  private:
    ...
    // my sort function -> bubble sort
    void _sort(vector<Point<Dim>>& newPoints, int d);

    // function that uses to build a tree
    void _createTree(KDTreeNode *& l, KDTreeNode *& r, vector<Point<Dim>>& pts, int d);

    // destructor helper
    void _free(KDTreeNode * cur);

    // function that uses to do copy job
    void _copy(KDTreeNode *& cur, KDTreeNode *& r);

    // helper function for findNearestNeighbor
    void _findNearestNeighborHelper(const Point<Dim>& query, KDTreeNode * cur, Point<Dim> & smallest, int d,
                                          bool & flag, double & distance) const;

    // function that gets the square of distance (also is the helper function for findNearestNeighbor)
    double getDistanceSqrt(const Point<Dim>& target, const Point<Dim>& cur) const;
};
